...
const indexView = quizzes =>
    ...
    quizzes.map(quiz =>
        ...
        <div class="divTableCell"><a href="/quizzes/${quiz.id}/play">${quiz.question}</a></div>
        <div class="divTableCell"><a href="/quizzes/${quiz.id}/edit"
                   class="button">Edit</a></div>
        <div class="divTableCell"><a href="/quizzes/${quiz.id}?_method=DELETE"
                   onClick="return confirm('Delete: ${quiz.question}')"
                   class="button">Delete</a></div>
                   </div>`).join("\n") +
    `</div></div>
...
const updateController = async(req, res, next) => {
    ...
    try {
        ...
        if (!quiz) next(new Error(`Quiz ${id} not found.`));

        quiz.question = question;
        quiz.answer = answer;
        await quiz.save({ fields: ["question", "answer"] });

        if (quiz) res.redirect(`/quizzes`); // Volvemos a la pantalla de la lista de quizzes
    } catch (err) {
        next(err)
    }
};
...
const destroyController = async(req, res, next) => {
    ...
    try {
        ...
        let quiz = await Quiz.destroy({ where: { id: `${id}` } });
        if (quiz) console.log(`Quiz ${id} borrado`);

        res.redirect(`/quizzes`); // Volvemos a la pantalla de la lista de quizzes
    } catch (err) {
        next(err)
    }
};
